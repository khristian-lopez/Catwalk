...
import RelatedProductCard from '../../client/src/components/RelatedProducts/RelatedProductCard.jsx';
import RelatedProductsList from '../../client/src/components/RelatedProducts/RelatedProductsList.jsx';

describe('RelatedProductCard component', () => {
  ...
  test('dynamically renders a related product\'s image', async () => {
    // render(<RelatedProductCard index={0} card={'related'} product={sampleProduct}/>);
    render(<RelatedProductsList currentProduct={sampleProduct}/>);

    const ancestor = await screen.findByTestId('rel-prod-card');
    const descendant = screen.getByTestId('prod-image');

    const thumbnail = "https://images.unsplash.com/photo-1477420143023-6a0e0b04b69a?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=300&q=80";

    expect(ancestor).toContainElement(descendant);
    expect(descendant).toHaveAttribute("src", thumbnail);
  })
})